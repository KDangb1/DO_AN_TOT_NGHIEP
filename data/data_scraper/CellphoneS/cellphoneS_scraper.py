...
class CellphoneS_scraper:
    ...
    #check the Popup banner is exist
    def check_popup_baner(self):
        try:
            self.driver.find_element(By.CLASS_NAME, "cancel-button-top")
        ...
        while not_show_all_items_yet:
            try:
                try:
                    ...
                    # popup banner
                    if self.check_popup_baner():
                        self.close_popup_banner()
                    # the page is showing all items
                    else:
                        not_show_all_items_yet = False
                        break
    ...
    def save_crawled_data(self, df_products, category, columns = ["product_name", "product_src_image", "product_price_new", "product_price_old"]):
        Path(self.data_storage_dir).mkdir(parents=True, exist_ok=True)
        
        df_products.to_csv(f"{self.data_storage_dir}/CellphoneS_{category.replace('-','_')}.csv", index=False, columns=columns, mode="w")
    ...
    def collect_products_by_category(self, category):
        ...
        try:
            ...
            self.save_crawled_data(df_products=df_all_items_in_page, category=category)
